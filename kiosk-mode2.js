class KioskMode {
  ...
  getConfig(lovelace) {
    ...
    try {
      ...
      const config = llConfig.kiosk_mode2 || {};
  ...
  styleExists(elem) {
    return elem.querySelector(`#kiosk_mode2_${elem.localName}`);
  }

  addStyle(css, elem) {
    if (!this.styleExists(elem)) {
      const style = document.createElement("style");
      style.setAttribute("id", `kiosk_mode2_${elem.localName}`);
      style.innerHTML = css;
  ...
  removeStyle(elements) {
    this.array(elements).forEach((elem) => {
      if (this.styleExists(elem)) elem.querySelector(`#kiosk_mode2_${elem.localName}`).remove();
    });
  }
}

// Overly complicated console tag.
const conInfo = { header: "%c≡ kiosk-mode2".padEnd(27), ver: "%cversion *DEV " };
const br = "%c\n";
